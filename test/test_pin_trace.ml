...
let%expect_test "what the host reads back in every scenario" =
  ...
  [%expect
    {|
    (uart_tx (cycles 9632) (reads ((0000))))
    (uart_rx (cycles 11875) (reads ((0055 00a3 00ff) (0000 000f 00f0) (0400))))
    (spi_master (cycles 10419) (reads ((0081 007e) (0400))))
    (i2c_logger (cycles 19325) (reads ((0000))))
    (wrapped_loop (cycles 7681) (reads ((0000))))
    (fifo_poll (cycles 12322)
     (reads
