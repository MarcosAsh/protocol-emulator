...
let%expect_test "uart tx" =
  ...
  [%expect
    {|
     ...
     4  set x, 7                     phase ?..?
     5  mov t, now                   phase ?..?
     6  set pins, 0                  phase 1
     7  add t, p                     phase 2
     8  wait t+                      phase -13..-12  slack 12..13
     9  out pins, 1                  phase -15
    10  jmp x--, 8                   phase -14
    11  wait t+                      phase -12  slack 12
    12  set pins, 1                  phase -15
...
let%expect_test "spi master" =
  ...
  [%expect
    {|
     ...
     8  out pins, 1 side 0           phase -7
     9  wait t+ side 0               phase -6..-4  slack 4..6
    10  in pins, 1 side 1            phase -7
    11  wait t+ side 1               phase -6  slack 6
    12  out pins, 1 side 0           phase -7
    13  jmp x--, 9                   phase -6
...
let%expect_test "i2c master" =
  ...
  [%expect
    ...
    36  mov pindirs, !y side 1       phase -6
    ...
    65  mov pindirs, !x side 1       phase -7
...
let%expect_test "a quarter of 5 is too short for the i2c dispatch" =
  ...
  [%expect
    ...
    36  mov pindirs, !y side 1       phase -3..-1
    ...
    65  mov pindirs, !x side 1       phase -3
