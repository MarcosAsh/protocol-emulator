...
(* The USB device firmware for the board's Python: the words for address 0, where the
   words that depend on the address sit, and those words for every address, so the
   Python never has to assemble anything or know how the constants are built. *)
let bit_period = 32
let addresses = 128
...
let () =
  ...
  print_endline "# Written by test/python/write_usb_firmware.ml; `dune promote` after a change.";
  print_endline "# Firmware.usb_device for every address, as the words that differ from address 0.";
  printf "\nBIT_PERIOD = %d\n\nCONFIG = {\n" bit_period;
